...
mod tests {
    ...
    fn func(src: &str) -> String {
        let list = crate::parser::parse_bash_script(src).expect("parses");
    ...
    fn output_reparses_to_the_same_tree() {
        for src in [
            ...
            let reparsed = crate::parser::parse_bash_script(&printed)
                .unwrap_or_else(|e| panic!("{printed}\n=> {e}"));
            let original = crate::parser::parse_bash_script(src).expect("parses");
