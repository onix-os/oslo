...
impl Session {
    pub fn new(text: &str, cursor: usize) -> Session {
        ...
        Session {
            buffer,
            vi: crate::ui::vi::enabled().then(super::vi::Vi::default),
        }
    }
...
/// can rebuild it — but only when [`crate::ui::prompt::generation`] says an input
...
pub fn read_line(
    ...
    let mut seen = crate::ui::prompt::generation();
        ...
        match session.apply(key, assist) {
            ...
            Step::Accept => {
                ...
                if session.vi.is_some() {
                    let shape = crate::ui::settings::current()
                        .vi
                        .cursors
                        .for_mode(crate::ui::vi::Mode::Insert);
