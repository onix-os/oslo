...
///
/// A [`crate::ui::paint::Panel`] plus the two rules that were got wrong when each widget
...
impl Inline {
    ...
    pub(crate) fn with_chrome(chrome: chrome::Chrome) -> Inline {
        ...
        Inline {
            ...
            panel: crate::ui::paint::Panel::at(0),
            chrome,
            took_the_screen,
        }
    }
...
/// while there is something animating. With a tick, [`crate::ui::term::Pressed::Timeout`]
/// means "nothing was typed, draw the next frame", which is a different answer from `Ended` and
/// has to stay that way.
pub(crate) fn awaited(keys: &mut crate::ui::term::Keys, tick: Option<i32>) -> Pressed {
    match tick {
...
fn caret_over(
    ...
    let (marked, style) = match shape {
        ...
        Cursor::Bar | Cursor::BlinkBar if under == ' ' => {
            ("▏".to_string(), crate::ui::theme::current().ui.accent)
        }
        Cursor::Underline | Cursor::BlinkUnderline => (
            ...
            Style {
                ...
                ..crate::ui::theme::current().ui.accent
            },
        ),
