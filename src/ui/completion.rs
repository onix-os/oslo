...
impl OsloHelper {
    ...
    pub fn candidates(&self, line: &str, pos: usize) -> (usize, Vec<CompletionCandidate>) {
        ...
        if let Some(wanted) = &crate::ui::settings::current().completion.sources {
            out.retain(|c| {
                c.kind
                    .as_deref()
        ...
        let by_name =
            crate::ui::settings::current().completion.sort == crate::ui::settings::Sort::Alpha;
        out.sort_by(|a, b| {
            if by_name {
    ...
    fn case_sensitive(&self) -> bool {
        crate::ui::settings::current().completion.case_sensitive
    }
