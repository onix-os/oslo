//! `oslo.nix_develop` — the environment of a Nix dev shell, without entering one.
//!
...
/// Install `oslo.nix_develop`.
pub fn install(oslo: &mut Table, env: &Arc<Mutex<Environment>>) {
    let env = Arc::clone(env);
    put(oslo, "nix_develop", move |_, args| {
        // `oslo.nix_develop()` means this directory's flake; a string names another installable,
        // exactly as `use flake ..#other` does.
        let installable = match args.first() {
            Some(Value::Str(_)) => text(&args, 1, "oslo.nix_develop")?.to_string(),
            _ => ".".to_string(),
        };
        ...
        let json = {
            ...
            crate::exec::eval_command_substitution(&mut guard, &command(&installable, &profile))
                .map_err(|e| LuaError::new(format!("oslo.nix_develop: {e}")))?
        };
        if json.trim().is_empty() {
            return Err(LuaError::new(
                "oslo.nix_develop: `nix print-dev-env` produced nothing — is nix installed, and \
