...
pub fn filter(rows: &[Record], expression: &str) -> (Vec<Record>, Option<String>) {
    ...
    let ast = match crate::lua::eval::parse(&source) {
        Ok(ast) => ast,
    ...
    let owned;
    let interp: &crate::lua::eval::Interp = match crate::lua::engine::interpreter_handle() {
        Some(handle) => {
            owned = handle;
            &owned
        }
        None => {
            owned = std::rc::Rc::new(crate::lua::eval::Interp::new("where"));
            &owned
        }
    };
    {
...
pub fn for_each(rows: &[Record], expression: &str) -> Option<String> {
    ...
    let interp: &crate::lua::eval::Interp = match crate::lua::engine::interpreter_handle() {
        ...
        None => {
            owned = std::rc::Rc::new(crate::lua::eval::Interp::new("each"));
            &owned
        }
    };
