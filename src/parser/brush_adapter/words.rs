...
pub(super) fn single_word(w: &ast::Word) -> Result<rush_ast::Word> {
    Ok(convert_word(w)?
        .into_iter()
        .next()
...
pub(super) fn convert_word(w: &ast::Word) -> Result<Vec<rush_ast::Word>> {
...
pub(super) fn convert_braced_word(w: &ast::Word) -> Result<Vec<rush_ast::Word>> {
    convert_braced_words_from_str(w.as_ref())
}

/// [`convert_braced_word`] for a word rush only has as text, such as an array literal element.
pub(super) fn convert_braced_words_from_str(word_str: &str) -> Result<Vec<rush_ast::Word>> {
...
pub(super) fn convert_words_from_str(word_str: &str) -> Result<Vec<rush_ast::Word>> {
    let trimmed = word_str.trim();
    if trimmed.is_empty() {
        return Ok(Vec::new());
    }
    ...
    loop {
        ...
        match lexer.next() {
            ...
            Ok(_) | Err(_) => return Ok(vec![rush_ast::Word::from_literal(trimmed)]),
        }
        if lexer.offset() == before {
            return Err(crate::error::ShellError::SyntaxError(format!(
    ...
    if words.is_empty() {
        Ok(vec![rush_ast::Word::from_literal(trimmed)])
    } else {
        Ok(words)
    }
}
