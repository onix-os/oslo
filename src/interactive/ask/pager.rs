...
fn cells(text: &str, width: usize) -> usize {
    ...
    for (offset, c) in text.char_indices() {
        let w = crate::interactive::prompt::printed_width(&c.to_string());
        if used + w > width {
            return offset;
        }
        used += w;
...
fn wrapped(line: &str, width: usize) -> Vec<String> {
    if width == 0 || crate::interactive::prompt::printed_width(line) <= width {
        return vec![line.to_string()];
    }
    let mut out = Vec::new();
    let mut current = String::new();
    for word in line.split(' ') {
        let would = crate::interactive::prompt::printed_width(&current)
            + usize::from(!current.is_empty())
            + crate::interactive::prompt::printed_width(word);
        if !current.is_empty() && would > width {
            out.push(std::mem::take(&mut current));
        }
        if crate::interactive::prompt::printed_width(word) > width {
            ...
            let mut rest = word;
            while crate::interactive::prompt::printed_width(rest) > width {
