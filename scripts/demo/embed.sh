...
#
# Idempotent: the block is delimited, so running this again after a re-record replaces the old
# player rather than stacking a second one under the heading. A document with no recording is left
# exactly as it is.
set -euo pipefail
...
while IFS=$'\t' read -r slug id; do
    ...
    if grep -qF "$BEGIN" "$doc"; then
        # Replace what is between the markers, keeping everything either side.
        ...
        # First time: under the opening paragraph, before the first "## " heading, so a reader sees
        # what the feature looks like before reading how it is built.
        awk -v block="$block" '
