...
-- **A right prompt that moves.** `oslo.builtin` has no spinner and no wish for one; this is a
-- segment list, which is the only prompt shape `every` reaches — an external prompt is a command
-- oslo runs, and running pixy ten times a second to move one glyph would be a process spawn per
-- frame, for ever, on a shell nobody is looking at.
--
-- So the *left* prompt stays pixy, untouched, and the right one is drawn here instead. It carries
-- what pixy's right zone carried — pod, directory, branch, vi mode — from the same sources: the
-- pod out of `$HEXE_POD_NAME`, which is where pixy reads it too. What is not reproduced is
-- `git_status`, which draws nothing in a clean tree anyway.
--
-- **Set this to false and pixy takes the right prompt back**, exactly as it was.
local ANIMATED_RIGHT_PROMPT = true

if ANIMATED_RIGHT_PROMPT then
  -- Braille, because it turns in place: every frame is one cell wide and none of them is a
  -- character that also means something, so the column beside it never moves.
  local FRAMES = { "⠋", "⠙", "⠹", "⠸", "⠼", "⠴", "⠦", "⠧" }
  local frame = 0

  oslo.prompt.right = {
    oslo.segment{ name = "spin", priority = 10, every = 120, render = function()
      frame = frame + 1
      return { { text = " " .. FRAMES[(frame % #FRAMES) + 1] .. " ", style = "prompt.aside" } }
    end },
    -- No `every` on any of the rest, which is the whole economy of it: these are rendered once and
    -- kept until something actually changes, however many times the spinner turns beside them.
    oslo.segment{ name = "pod", priority = 20, render = function()
      local pod = oslo.env.get("HEXE_POD_NAME")
      if not pod or pod == "" then return "" end
      return { { text = "| " .. pod .. " |", style = "prompt.aside" } }
    end },
    oslo.segment{ name = "directory", priority = 90, render = function(ctx)
      return { { text = " " .. oslo.path.shorten(oslo.path.home(ctx.cwd), 3) .. " ", style = "prompt.cwd" } }
    end },
    oslo.segment{ name = "branch", priority = 60, render = function(ctx)
      if not ctx.branch then return "" end
      return { { text = " " .. ctx.branch .. " ", style = "prompt.git" } }
    end },
    oslo.segment{ name = "vimode", priority = 30, render = function(ctx)
      if not ctx.vimode or ctx.vimode == "" then return "" end
      return { { text = " " .. ctx.vimode:sub(1, 1):upper() .. " ", style = "prompt.cwd" } }
    end },
  }
end
