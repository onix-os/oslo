...
// **Vendored code is not restyled.** oslo builds with `-D warnings`, and this crate is somebody
// else's, kept close to upstream so that a fix can still be read against it. Silencing the lint at
// the root is the honest arrangement: the alternative is a diff full of edits that are about
// oslo's house style rather than about this parser, which is exactly what makes a vendored crate
// impossible to update later.
