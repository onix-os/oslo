...
//! that `oslo macros show` can mark it rather than leave you wondering why your config stopped
//! working.
//!
...
use oslo_base::macros::{Kind, snapshot};
use oslo_shell::env::Environment;
use std::sync::{Arc, Mutex};

/// Apply what `oslo macros` has stored, and record what the config had defined first.
...
///
/// A label for `oslo macros show`, so it can say "this shadows config.lua". Read from a file rather
...
fn record_configured(env: &Arc<Mutex<Environment>>) -> Option<()> {
    ...
    let dir = oslo_base::macros::directory()?;
