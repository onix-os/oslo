//! Bridge from `brush_parser`'s AST to oslo's own [`oslo_base::ast`].
//!
...
//! - `time -p` is carried as plain `time`: [`oslo_base::ast::Pipeline`] records *that* a pipeline is
...
pub fn parse_bash_script(script: &str) -> Result<oslo_ast::CommandList> {
    ...
    // could be produced. See [`oslo_base::nesting`].
