...
/// Point the tab directory at a scratch path for the duration of one test.
///
...
///
/// **One lock for every test in this subtree, not one per module.** `OSLO_TAB_DIR` is
...
pub(crate) fn scratch() -> (tempfile::TempDir, std::sync::MutexGuard<'static, ()>) {
    ...
    unsafe { std::env::set_var("OSLO_TAB_DIR", dir.path().join("tab")) };
    (dir, guard)
}
