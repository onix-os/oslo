...
/// The whole point: the shell inside the tab turned the protocol on, so the chord arrives spelled
...
fn other_sequences_are_left_alone() {
    ...
    // reads what was *typed*: what the tab prints is copied out without being looked at.
...
///
/// `f4` and the arrows arrive as escape *sequences*, and a program inside a tab can send the same
/// bytes; swallowing them would make the tab eat output that was never a keystroke.
#[test]
