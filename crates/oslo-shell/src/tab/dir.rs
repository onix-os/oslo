//! Where tabs live, and proving it is ours before anything is written there.
//!
//! `/tmp/oslo-$UID/tab`, `0700`. `/tmp` is chosen over `$XDG_RUNTIME_DIR` because systemd deletes
//! the latter at last logout unless lingering is enabled, and a tab dying at logout defeats the
...
/// The mode a tab directory must have, and is created with. Nothing shared, nothing readable.
const REQUIRED: u32 = 0o700;

/// Where tabs live: `$OSLO_TAB_DIR`, or `/tmp/oslo-$UID/tab`.
///
/// The variable is what `oslo.tab.dir` writes, and what a test uses to stay out of the real one.
pub fn path() -> PathBuf {
    if let Some(set) = std::env::var_os("OSLO_TAB_DIR").filter(|value| !value.is_empty()) {
        return PathBuf::from(set);
    }
    default_root().join("tab")
}
...
fn guarded_parent() -> Option<PathBuf> {
    (path() == default_root().join("tab")).then(default_root)
}

/// Open the tab directory, creating it if it does not exist, and refuse anything suspicious.
///
...
pub fn open_checked() -> io::Result<OwnedFd> {
    ...
    // directory. Proven ours and `0700`, nothing can reach inside it, so `tabs` within it needs
