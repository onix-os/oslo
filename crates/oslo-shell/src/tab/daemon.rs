//! One process in front of every tab, for `oslo.tab.daemon = true`.
//!
//! ```text
//!   client ──► daemon.sock   0x10                    which tabs are there?
//!                            0x11 len(u16be) name…   put me in this one
//!
//!   after an attach the connection is a pipe: whatever the tab says, and whatever is typed at it,
//!   spliced byte for byte. The daemon reads none of it.
//! ```
//!
//! # What it is for, given the other backend works
//!
//! One socket instead of one per tab, and one process that knows the whole list without reading a
...
//! client forks a keeper without one — so a machine with no tabs has no oslo process running, which
...
/// Ask which tabs there are.
const LIST: u8 = 0x10;
/// Put me in this tab, making it if it is not there.
...
fn answer(mut client: UnixStream) -> io::Result<()> {
    ...
    match request {
        ...
        // The daemon makes the tab, so the client never forks a shell in this mode — and then gets
        ...
        Request::Attach(name) => {
            ...
            let tab = super::client::connect(&store::Paths::new(&name).sock(), &name)?;
            splice(client, tab)
        }
    }
}
...
fn splice(mut client: UnixStream, mut tab: UnixStream) -> io::Result<()> {
    ...
    loop {
        let (from_client, from_tab) = {
            let mut fds = [
                ...
                PollFd::new(tab.as_fd(), PollFlags::POLLIN),
            ];
        ...
        if from_client && !copy(&mut client, &mut tab, &mut buffer) {
            return Ok(());
        }
        if from_tab && !copy(&mut tab, &mut client, &mut buffer) {
            return Ok(());
        }
    }
}
