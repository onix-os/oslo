...
fn a_dead_tab_is_swept_by_whoever_lists() {
    ...
    let tabs = list().expect("list");
    assert!(tabs.is_empty(), "a dead tab is not listed: {tabs:?}");
    assert!(
...
/// A live tab is listed, with what it said about itself.
#[test]
fn a_live_tab_is_listed_with_its_meta() {
    ...
    assert_eq!(tabs[0].0, "gamma");
    assert_eq!(tabs[0].1.cwd, "/tmp/somewhere");
    assert_eq!(tabs[0].1.pid, 42);
    drop(held);
}
...
/// A torn `.meta` costs the decoration, never the tab.
#[test]
fn an_unreadable_meta_still_lists_the_tab() {
    ...
    let tabs = list().expect("list");
    assert_eq!(tabs.len(), 1, "still listed");
    assert_eq!(tabs[0].1.pid, 0, "with the fields at their defaults");
    drop(held);
}
...
/// A file in the directory that is not a tab is not read as one.
#[test]
