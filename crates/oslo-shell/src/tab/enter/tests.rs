...
/// are allowed to become a tab, and what `$TAB` says about where we are.
#[test]
...
/// `$TAB` is what a prompt reads, and the empty string has to mean "not in one" — a variable that
/// is set but empty is the shape an `export TAB=` leaves behind.
#[test]
