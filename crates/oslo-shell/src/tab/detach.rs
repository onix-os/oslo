//! The chord that steps out of a tab, in both of the encodings a terminal may send it in.
//!
//! # Why one byte is not enough
//!
//! A tab hands a pty to a shell, and that shell asks the terminal for the Kitty keyboard protocol —
//! `\x1b[>5u`, so it can tell `^I` from Tab in its line editor. That request is not addressed to the
...
//!
//! So the shell inside a tab quietly changes how the key that leaves the tab is spelled. A client
...
/// prefix; a proxy that swallowed it would break both inside every tab.
