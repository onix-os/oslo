...
fn a_readable_directory_is_refused() {
    ...
    let err = open_checked().expect_err("a 0755 scratch directory must be refused");
...
fn the_default_is_under_tmp_and_per_user() {
    ...
    unsafe { std::env::remove_var("OSLO_SCRATCH_DIR") };

    let dir = path();
    let uid = nix::unistd::getuid().as_raw();
    assert_eq!(dir, PathBuf::from(format!("/tmp/oslo-{uid}/scratch")));
}
