use super::*;
use crate::scratch::{dir, scratch, store};
use std::ffi::CString;
use std::io::{Read, Write};
use std::time::{Duration, Instant};

/// Run a program in the scratch instead of a shell — the keeper does not care which, and a test that
...
/// End a scratch the way `scratch -k` will: signal the keeper and wait for it, so the next test does not
...
/// Make a scratch whose shell is `run`, and answer with the keeper's pid.
///
...
/// **The whole mechanism, end to end**: a scratch exists, something is running in it, and a client can
...
fn a_tab_runs_a_program_and_a_client_can_talk_to_it() {
    ...
    assert!(until(|| store::alive("alpha")), "the scratch never came up");
    assert!(
    ...
    let mut client = std::os::unix::net::UnixStream::connect(store::Paths::new("alpha").sock())
        .expect("connect");
    client
        .write_all(&crate::scratch::wire::data(b"hello\n"))
        .expect("write");
    client
    ...
    // What the scratch printed is on disk too, which is what `tail -f` reads and what a reattach
    // replays.
...
/// The scratch ends when what is running in it ends, and tidies up after itself.
#[test]
fn a_tab_ends_with_its_program() {
    ...
    assert!(
        ...
        "the scratch outlived its program"
    );
    assert!(
...
/// matters is that something *inside* the scratch sees it.
#[test]
fn a_resize_reaches_the_program_inside() {
    ...
    let mut client = std::os::unix::net::UnixStream::connect(store::Paths::new("epsilon").sock())
        .expect("connect");
    client
        .write_all(&crate::scratch::wire::resize(31, 121))
        .expect("resize");
