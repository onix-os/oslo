//! The terminal side: what you are looking at while a scratch is attached.
//!
...
pub enum Left {
    /// The detach key was pressed. The scratch is still running.
    Detached,
    /// The scratch ended — the shell inside exited, or the keeper went away.
    Ended,
}

/// Attach to `name` until the scratch ends or the detach key is pressed.
///
...
pub fn attach(
    ...
    // **The screen belongs to one scratch at a time.** Whatever is on it was written by somebody else —
    // the shell you pressed the key in, or the scratch you just left — and replaying this scratch's output
    // on top of it interleaves two sessions into one screen that belongs to neither.
    take_screen();
    // What the scratch printed while nobody was watching, so an attach lands on the screen it left
    ...
    // The scratch has no idea how big this terminal is until it is told, and it has just changed hands.
    ...
    // whoever was there, and leaving a scratch is not a reason to lose it.
    give_screen_back();
    drop(restore);
    outcome
}
...
///
/// **A scratch exists before it listens.** `spawn` returns as soon as the fork has happened, and the
/// keeper binds its socket a few syscalls later — so a caller that made a scratch and attached to it in
/// the same breath would fail against one that is perfectly healthy. Retrying briefly is the
/// difference; a scratch that is genuinely not there fails just as surely, only a moment later.
...
/// Take the screen for the scratch, on the terminal's second one.
///
/// **The alternate screen rather than a clear.** Both give the scratch a blank screen to itself, and
/// only this one gives the first screen *back*: the terminal keeps it, scrollback and all, and
/// redraws it untouched on the way out. Clearing instead — which is what scratch-rs does, since it has
/// no shell of its own to return to — threw away the scrollback of the session the key was pressed
/// in, which is somebody else's work and not a scratch's to discard.
///
/// **And not `ESC c` to tidy it, however tempting.** A full reset is what scratch-rs sends, and on a
/// real terminal it takes the alternate screen down with it and empties the scrollback — undoing
/// the one thing this is for. What is left is the state a scratch could otherwise inherit and draw
...
/// Tell the scratch how big this terminal is.
