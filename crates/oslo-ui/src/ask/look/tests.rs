...
fn a_full_width_row_reaches_the_edge() {
    ...
    assert_eq!(crate::prompt::printed_width(&rows[0]), 40, "{rows:?}");
}
...
fn a_trail_keeps_its_room() {
    ...
    assert_eq!(crate::prompt::printed_width(&drawn[0]), 40, "{drawn:?}");
}
...
fn the_meta_widths_do_not_change_as_the_list_scrolls() {
    ...
    let column_of =
        |row: &str| crate::prompt::printed_width(row.split("a command").next().unwrap_or(""));
...
fn the_sweep_moves_without_moving_anything_else() {
    ...
    assert_eq!(
        crate::prompt::printed_width(&first),
        crate::prompt::printed_width(&later),
        "{first:?} {later:?}"
    );
}
