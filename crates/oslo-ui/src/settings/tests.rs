...
/// `oslo.tab` is read whether or not the build can act on it, so one config works on a machine
/// with the feature and a machine without.
#[test]
fn the_tab_block_is_read() {
    let (settings, problems) = settings_from(
        r#"oslo = { tab = { key = "ctrl-g", daemon = true, dir = "/run/x", log_bytes = 4096 } }"#,
    );
    assert!(problems.is_empty(), "{problems:?}");
    assert_eq!(settings.tab.key, "ctrl-g");
    assert!(settings.tab.daemon);
    assert_eq!(settings.tab.dir, "/run/x");
    assert_eq!(settings.tab.log_bytes, 4096);
}

#[test]
fn the_tab_default_is_ctrl_backslash_and_no_daemon() {
    let (settings, problems) = settings_from("oslo = {}");
    assert!(problems.is_empty(), "{problems:?}");
    assert_eq!(settings.tab, Tab::default());
    assert_eq!(settings.tab.key, "ctrl-\\");
    assert!(!settings.tab.daemon);
}

/// A key nothing can name is reported against the line that wrote it, and the default is kept —
/// the alternative is a tab with no way out.
#[test]
fn a_tab_key_that_is_not_a_key_is_reported() {
    let (settings, problems) = settings_from(r#"oslo = { tab = { key = "wat" } }"#);
    assert_eq!(problems.len(), 1, "{problems:?}");
    assert!(problems[0].contains("oslo.tab.key"), "{problems:?}");
    assert_eq!(settings.tab.key, Tab::default().key);
}
