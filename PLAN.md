...
[tab-rs](https://github.com/austinjones/tab-rs). Behind a cargo feature, off by default, like `ssh`
and `vista`.

**One key is the whole interface.** `ctrl+\` opens the finder, and it does so in the same way
whether or not you are in a tab. There is no `tab` command, no subcommands and no arguments. A shell
never becomes a tab on its own; nothing happens until the key is pressed.
...
|---|---|---|
| outside a tab | `ctrl+\` | the finder: every tab, plus a `create <query>` row |
| outside, in finder | Esc | nothing — back to your prompt, line intact, no tab made |
| outside, in finder | pick one | attach to it |
| outside, in finder | type a new name, Enter | create it, attach |
| inside a tab | `ctrl+\` | the same finder, **this tab listed too** |
| inside, in finder | Esc | detach — back to the shell you pressed the key in, tab still running |
...
| inside, in finder | type a new name, Enter | create and attach; the old tab keeps running |

Nothing is ever killed by navigating. `$TAB` holds the current name, for the prompt.

## The key

`ctrl+\` by default, `oslo.tab.key`. There is no two-key sequence — Esc in the finder is the
disconnect, so one binding covers everything.

**It is handled in two different places, and they are not interchangeable.** Outside a tab the shell
owns the terminal, so this is an editor keybinding like any other, fired at the prompt. Inside a tab
...
**It must be matched in two encodings.** The shell inside a tab asks the terminal for the Kitty
keyboard protocol (`\x1b[>5u`, so its line editor can tell `^I` from Tab). That request is output, so
...
Whatever the key is, it is swallowed from every program running inside a tab. That is the cost, and
...
oslo.tab.daemon = true    -- a registry process, as tab-rs does it
```
...
|---|---|---|
| processes | one keeper per tab | one daemon + one pty process per tab |
| registry | the runtime directory | the daemon |
| when no tabs exist | nothing running | nothing running |
| transport | unix socket per tab | one unix socket |

**Unix socket for the daemon too**, not tab-rs's localhost TCP port and auth token. On Linux the
...
```text
  ...
  tab ──unix socket──► keeper           tab ──unix socket──► daemon
                         │ pty                                 │
                         ▼                                     ├─► pty proc ─► oslo
                       oslo                                    └─► pty proc ─► oslo
...
`crates/oslo-shell/src/tab/`, feature-gated at the module. `dir.rs`, `store.rs`, `keeper.rs`,
...
The runtime directory is `$OSLO_TAB_DIR`, else `/tmp/oslo-$UID/tab`, mode 0700, ownership checked on
...
The row appears only when the query matches no existing entry, exactly as tab-rs does it
...
Each ends with `make verify` green and is its own commit. Branch `feat/tab` off `develop`.
...
2. Cargo feature `tab`; `oslo.tab` settings (`key`, `daemon`, `dir`, `log_bytes`). A config carrying
...
9. The daemon backend behind `oslo.tab.daemon`.
10. `docs/features/tab.md`.

`$TAB` is set in the process environment **and** in the shell's own variable table — setting only
the first leaves `$TAB` empty in shell while `os.getenv` in the prompt can see it.
...
- `kill -9` the client: the tab survives and reattaches. `kill -9` the keeper: it is swept.
...
**Closing a tab has no gesture yet.** The finder never kills anything, and there is no command, so
as it stands a tab ends only when its shell exits. That is consistent — `exit` is how you close a
shell — but it leaves no way to reach a tab whose shell is wedged. A key in the finder is the
